    ...
    <div class="section-view__content">
      ...
      <div v-else class="section-view__item-grid">
        <div
          ...
          :class="{ 'section-item-card--edit': isEditMode, 'section-item-card--dragging': dragIndex === index }"
        ...
        >
          ...
          <div v-if="item.images?.length" class="section-item-card__images">
            <img v-for="(img, i) in item.images" :key="i" :src="img" :alt="item.title" class="section-item-card__image" />
          </div>
...
export default {
  ...
  setup(props) {
    ...
    watch(() => state.sectionItems['skills'], (newItems) => {
      if (!isEditMode.value) localSkillItems.splice(0, localSkillItems.length, ...(newItems ?? []))
    }, { deep: true })

    const displayItems = computed(() => {
      if (isSkillsSection.value) return localSkillItems

      const rawItems = state.sectionItems[props.sectionId] ?? []
      return [...rawItems].sort((a, b) => parseDateToNumber(b.startDate) - parseDateToNumber(a.startDate))
    })
    ...
    function onDragStart(index) { if (isEditMode.value && isSkillsSection.value) dragIndex.value = index }

    function onDragOver(targetIndex) {
      if (!isEditMode.value || !isSkillsSection.value || dragIndex.value === null || dragIndex.value === targetIndex) return
    ...
    return {
      state, authState, currentSection, displayItems,
      isSkillsSection, isEditMode, isModalOpen, editingItem, dragIndex,
