  ...
  >
    <div v-for="field in visibleFields" :key="field" class="form-group">
      ...
      <div v-if="field === 'tags'" class="tags-input-area">
        <input v-model="tagsInput" class="form-input" type="text" placeholder="쉼표로 구분. 예) Vue.js, Java, Spring Boot" />
      ...
      <div v-else-if="field === 'images'" class="image-upload-area">
        ...
        <div v-if="formData.images.length" class="image-preview-list">
          <div v-for="(image, index) in formData.images" :key="index" class="image-preview-item">
            ...
            <button class="image-preview-remove" @click="removeImage(index)"><SvgIcon name="close" /></button>
          </div>
        </div>
      </div>
    ...
    <template #footer>
      ...
      <button class="btn btn--primary" @click="handleSubmit">{{ isEditMode ? '저장' : '추가' }}</button>
    </template>
  </BaseModal>
</template>
...
const FIELD_LABEL_MAP = {
  title: '제목 *', subtitle: '부제목', context: '내용',
  startDate: '시작일', endDate: '종료일', tags: '사용 기술 (쉼표 구분)', images: '이미지',
}
const FIELD_PLACEHOLDER_MAP = {
  title: '제목을 입력하세요', subtitle: '부제목을 입력하세요', context: '내용을 입력하세요',
  startDate: 'YYYY.MM', endDate: 'YYYY.MM (미입력 시 현재)',
  tags: 'Vue.js, Java, Spring Boot', images: '',
}
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const tagsInput = ref(props.editItem?.tags?.join(', ') ?? '')
    ...
    function handleSubmit() {
      ...
      visibleFields.value.forEach((field) => {
        if (field === 'images') itemData.images = [...formData.images]
        else if (field === 'tags') itemData.tags = [...parsedTags.value]
        else itemData[field] = formData[field]
      })

      if (isEditMode.value) updateSectionItem(props.sectionId, props.editItem.id, itemData)
      else                  addSectionItem(props.sectionId, itemData)

      emit('close')
    }

    return {
      formData, tagsInput, parsedTags, visibleFields, sectionTitle,
      errorMessage, isEditMode, imageInputRef,
      fieldLabelMap: FIELD_LABEL_MAP, fieldPlaceholderMap: FIELD_PLACEHOLDER_MAP,
      triggerImageUpload, handleImageUpload, removeImage, handleSubmit,
    }
  },
}
</script>
...
.form-input {
  height: clamp(28px, 3.5vh, 44px); padding: 0 0.8vw; border-radius: 0.4vw;
...
.form-textarea {
  padding: 0.8vh 0.8vw; border-radius: 0.4vw; border: 1px solid var(--border);
  background-color: var(--bg-card-secondary); color: var(--text-primary);
...
.tags-preview { display: flex; flex-wrap: wrap; gap: 0.3vw; }
.tag-badge {
  display: inline-block; padding: 0.2vh 0.5vw; border-radius: 0.3vw;
  background-color: #EFF6FF; color: #1D4ED8; font-size: var(--font-label);
...
.image-upload-btn {
  ...
  font-size: var(--font-body); align-self: flex-start; transition: border-color 0.2s, color 0.2s;
}
...
.image-preview-thumb { width: clamp(50px, 5vw, 80px); height: clamp(50px, 5vw, 80px); object-fit: cover; border-radius: 0.4vw; border: 1px solid var(--border); }
.image-preview-remove {
  position: absolute; top: -0.5vh; right: -0.3vw;
  width: clamp(16px, 1.5vh, 22px); height: clamp(16px, 1.5vh, 22px);
  border-radius: 50%; background-color: #EF4444; color: #fff;
  display: flex; align-items: center; justify-content: center; border: none;
}
.image-preview-remove:hover { background-color: #DC2626; }

.btn { height: clamp(28px, 3.5vh, 44px); padding: 0 1.2vw; border-radius: 0.4vw; font-size: var(--font-body); font-weight: 500; transition: background-color 0.2s; }
