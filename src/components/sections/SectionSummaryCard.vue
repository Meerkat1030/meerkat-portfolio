<template>
  <div class="section-summary-card">
    ...
    <div v-else class="section-summary-card__preview">
      ...
      <span v-if="hasMoreItems" class="section-summary-card__preview-more">+{{ items.length - 3 }}개 더</span>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props) {
    ...
    const hasMoreItems = computed(() => props.items.length > 3)
...
.section-summary-card__preview-more {
  font-size: var(--font-label);
  color: var(--text-secondary);
  padding-left: 0.75vw;
}
